...
| `malformed_shape` — with the named-path kinds `malformed_open_thread`, `malformed_dropped_receipt`, `malformed_treatment_landscape`, `malformed_promotion_proposal` | A node whose container type, required keys, or enum value departs from [07]. Enforced by **one declarative table** (`ISSUE_SHAPE_V2`) walked generically, never a function per field — so the gate's coverage cannot drift from the contract it enforces, and a shape added to [07] that nobody wired up is a failing drift test rather than a post-publication crash. | Emit the shape [07] specifies. |
